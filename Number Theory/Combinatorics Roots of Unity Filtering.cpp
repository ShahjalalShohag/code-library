...
poly power(poly a, long long k, poly mod) {
	poly ans({1});
	while (k) {
...
combi C(N);
// Calculate \sum_ncr(n, i), 0 <= i <= n, i % m = r
// P(m log^2 m)
// ans = constant term of x^(m - r) * (1 + x) ^ n % (x^m - 1)
int32_t main() {
	ios_base::sync_with_stdio(0);
	cin.tie(0);
    long long n; int m, r, mod; cin >> n >> m >> r;
    poly ans({1, 1}), M(m + 1, 0);
    M.a[m] = 1; M.a[0] = -1;
    ans = power(ans, n, M);
    ans = ans.mul_xk(m - r) % M;
    cout << ans[0] << '\n';
    mint res = 0;
    for (int i = 0; i <= n; i++) {
    	if (i % m == r) {
...
// https://codeforces.com/gym/280379/problem/A