...
struct MaximumClosure { //answer could be an empty set and could be disconnected
	...
	vector<int> vec, idx; vector<long long> w; 
	MaximumClosure(int _n) {
		n = _n ;
		vis.assign(n + 1, 0); idx.assign(n + 1, 0);
		g.resize(n + 1); r.resize(n + 1); G.resize(n + 1);
		vec.clear(); w.assign(n + 1, 0);
	}
	...
	long long solve() {
	    ...
	    Dinic F(scc + 3); int s = scc + 1, t = scc + 2;
	    for (int u = 1; u <= n; u++) {
	        for (auto v: g[u]) {
	            if(idx[u] != idx[v]) {
