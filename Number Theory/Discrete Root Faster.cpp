...
long long peth_root(long long a, long long p, int e, long long mod) {
    ...
    long long pe = power(p, e, mod);
    long long ans = power(a, ((pe - 1) * inverse(q, pe) % pe * q + 1) / pe, mod);
    long long c = 2;
    ...
    long long mul = power(c, v * power(p, s - 1, mod - 1) % (mod - 1), mod);
    for (int i = 0; i <= v; ++i) {
        mp[add] = i;
        add = add * mul % mod;
    }
    ...
    for (int i = e; i<s; ++i) {
        long long err = inverse(power(ans, pe, mod), mod) * a % mod;
        long long target = power(err, power(p, s - 1 - i, mod - 1), mod);
        for (int j = 0; j <= v; ++j) {
...
// Complexity: O(p ^ (1 / 4))
long long DiscreteRoot(long long a, long long k, long long p) {
    if (k > 0 && a == 0) return 0;
    k %= p - 1;
...
int32_t main() {
    ...
    int t; cin >> t;
    while (t--) {
        long long k, a, p; cin >> k >> a >> p;
        cout << DiscreteRoot(a, k, p) << '\n';
    }
}
