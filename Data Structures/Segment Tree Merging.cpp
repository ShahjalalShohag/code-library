...
struct STM {
    ...
    int T;
    int id[N * 30];
    STM() { T = 0; for (int i = 1; i < N * 30; i++) id[i] = i; }
    inline int new_node() {
        int cur = id[++T];
...
int main() {
    ...
    while (q--) {
        ...
        split(*(--it), r);
        //get the range
        set<int>::iterator L = cur.lower_bound(l), R = cur.upper_bound(r); --R;
        int nw = *L;
        vector<int> v;
        while (L != R) {
            ++L; int k = *L; v.push_back(k);
            if (L == R) break;
        }
        for (int i = 0; i < v.size(); i++) merge(nw, v[i]);
        ty[nw] = type;
    }
    int i; scanf("%d", &i); //query for the i-th position of the sequence
